...
def init_dist(N):
    """
    INPUT:
    n0 = total aerosol number density (/m3)
    n: initial distribution array returned from function
    Generates initial disctribution with the mean and standard variation
    specified.
    """

    #    ind=math.log(vi/v0)/math.log(bin_ratio)
    #    ind=int(ind)
    #    n[ind]=1.0E9

    #    for i in range(NB):
    #        n[i] = n0*dv[i]/vm*math.exp(-vol[i]/vm)
    lnf = np.zeros(NB)
    s = math.log(2.0)
    # N < NB to allow for aerosol growth
    for i in np.linspace(1, NB, NB, dtype=int):
        lnf[i-1] = 1.0/(math.sqrt(2.0*math.pi)*s)/vol[i-1] * \
            math.exp(-(math.log(vol[i-1]/vm))**2/(2.0*s*s))*dv[i-1]
    return lnf
...
paramf.readline()
param = paramf.readline()
ditxt, d1txt, d2txt, dmedtxt = param.split()
paramf.close()
...
d1 = float(d1txt)*1e-6
d2 = float(d2txt)*1.0E-6  # micro meter
v0 = 1.0/6.0*Pi*d0**3
...
br = (v2/v0)**(1.0/float(NB-1))
Nless = 1/np.log(br) * np.log(v1/v0)
# br = (v2/v0)**(1.0/float(NB-1-Nless))  # NB-1# bin ratio

# dlogv = math.log(br)
dv = np.zeros(NB)
# dv is v_right_bound-v_left_bound  =
for i in np.linspace(1, NB, NB, dtype=int):
    # dv[i-1] = v0*(br**i)/2.0*(br-1.0/br)
    dv[i-1] = v0*(br**i)*(1.0-1.0/br)
    # dv[i-1]= 2.0*v0*br**i*(br-1)/(1+br)
    # dv[i]=dlogv


"""
TEST and plot kernel to validate with SOAR report plot
NB=500
dl=1.0E-3*1.0E-6
du=100.0*1E-6
vl=Pi/6.0*(dl)**3
vu= Pi/6.0*(du)**3
dx=1.0E-6
bin_ratio=(vu/vl)**(1.0/float(NB-1))
ttg=[i for i in range(NB)]
ttb=[i for i in range(NB)]
ttd=[i for i in range(NB)]
tti=[i for i in range(NB)]
x=tti[:]
K0=1.0E6  # cm3
for i in range(NB):
    v1=Pi/6.0*(dx)**3
    v=vl*bin_ratio**i
    x[i]=(6.0/Pi*v)**(1.0/3.0)*1E6
    ttg[i]=KG(v,v1)*K0
    ttb[i]=KB(v,v1,300.0)*K0
    ttd[i]=KTD(v,v1)*K0
    tti[i]=KTI(v,v1)*K0
print (K0)
ax=plt.gca()
ax.set_xlim(xmin=3.0E-3,xmax=3.0E1)
ax.set_ylim(ymin=1.0E-13,ymax=1.0E-5)
plt.loglog(x,ttg,'green',x,ttb,'brown',x,ttd,'red',x,tti,'blue')
#plt.loglog(x,ttd,'green',x,tti,'+')
plt.grid(which='major',axis='both')
plt.legend()
plt.savefig("Kernel_SOAR.jpg")
sys.stdin.readline()
sys.exit("End")
"""
# END TEST
...
# vol = np.array([fv(i) for i in range(NB)])   # get bin volumes from bin ratio

lnf = init_dist(NB-Nless)


print("\n distribution check", sum(lnf))
print("\n")

n0c = total_mass/rhom/sum(lnf*vol)/V
n0 = lnf * n0c

# Test problem
# n0[0] =1.0E12/0.08  #d ln(Dp)

# sys.stdin.readline()

# mgpcum = n0c*rhom*vm*1.0E+6/ddv
# print("n0=%E,mg/cum=%E" % (n0, mgpcum)
...
if fl_iso == 0:
    ...
    plt.ylabel("Number of Aerosols")
    plt.xlabel("Aerosol Diameter (micron)")
    plt.legend()
    plt.minorticks_on()
    plt.grid(which='major', axis='both')
    plt.grid(which='minor', axis='both', color='#ced1cc')
